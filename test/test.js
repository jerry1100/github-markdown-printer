...
const testCases = [
  ...
  new TestCase(
    'readme file',
    'https://github.com/jerry1100/github-markdown-printer/blob/master/test/sample/README.md'
  ),
  new TestCase(
    'wiki',
    'https://github.com/jerry1100/github-markdown-printer/wiki'
  ),
];
...
async function getTestResults(testCases) {
  const results = testCases.map(
    async ({ testName, urlToTest }) => await tryRunTest(testName, urlToTest)
  );
...
async function runTest(testName, urlToTest) {
  ...
  const numDiffPixels =
    await testRunner.doesScreenshotMatchExpectedAndGenerateDiff(
  ...
  if (didPass) {
    ...
    console.log(
      `[${testName}] Failed - actual screenshot differs from expected by ${numDiffPixels} pixels`
    );
  }
